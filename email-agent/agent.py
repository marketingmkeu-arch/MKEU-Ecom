...
def get_customer_first_name(order):
    if order:
...
def get_widerruf_context(order):
    ...
    shipped = (order.get("fulfillment_status") or "") in ("fulfilled", "partial")
...
def mark_as_read(token, email_id):
    headers = {"Authorization": f"Bearer {token}", "Content-Type": "application/json"}
    requests.patch(
        f"{GRAPH_URL}/users/{EMAIL_ADDRESS}/messages/{email_id}",
        headers=headers,
        json={"isRead": True},
    )
